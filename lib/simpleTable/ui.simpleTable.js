...
angular.module('ui.simpleTable', [])
  .directive('inputPhone', function($log){
    // Runs during compile
    return {
      scope: {
        defaultCountry : '='
      ...
      controller : function($scope, $element, $attrs) {
        $log.debug('Controller - input phone');
      },
      ...
      link: function($scope, iElm, iAttrs, controller) {
        $log.debug('linking - input phone');
        iElm.addClass('input-group');
        ...
        $log.debug('listContainer : ', listContainer);
        ...
        angular.forEach(countries, function(value, key){
          ...
          if (iAttrs.defaultCountry) {
            if (value.code === iAttrs.defaultCountry) {
              prefix.html(value.dial_code);
            }
          }
          ...
          aTag.click(function(ev) {
            var countryCode = $(ev.currentTarget).html();
            prefix.html(countryCode);
            ulTag.toggleClass('hidden');
          });

          aTag.html(value.name + ' ' + value.dial_code);
          liTag.append(aTag);
          ulTag.append(liTag);

        });

        listContainer.append(ulTag);
        listContainer.insertAfter(iElm);

        prefix.click(function(ev) {
          $log.debug('Toggle list');
          ulTag.toggleClass('hidden');
        });
